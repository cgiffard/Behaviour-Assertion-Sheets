...
module.exports = function(parseStatement) {
	...
	function trackGrouping(char,prev) {
		...
		if (groupingChars[stackTop] === char)
		...
		if (prev.match(/[a-z0-9]/i)) {
			if (stringDelimiters[char])
				return false;
			
			if (regexDelimiters[char] && !inRegularExpression())
				return false;
		}
		
		// This is a new character which will add a new enclosure context.
		if (groupingChars[char] && !stringDelimiters[stackTop])
			return groupingStack.push(char);
	}
	...
	while (pointer < parseStatement.length) {
		...
		if (!escapeSequences[character] || prevCharWasEscape())
