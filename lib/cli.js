...
// Block for optionalDependency iconv
// This'll have a cry if you don't have it, but it should still work.
var iconv = null;
try {
	iconv = require("iconv");
} catch(e) {
	console.error(
		"Warning: ".red +
		"It appears " + "iconv".white + " was not properly installed.\n" +
		"Unless you install it, you may be unable to correctly test pages " +
		"encoded in anything other than UTF-8.");
}

function utf8(buffer, sourceEncoding) {
	// If iconv wasn't present...
	if (!iconv) return buffer;

	try {
		return (
			(new iconv.Iconv('UTF-8', sourceEncoding))
				.convert(buffer));
...
basCLI
	...
	.option("-p --password <password>",		"Password for HTTP Basic Auth (crawl)")
	.option("-i --interval <integer>",		"Request interval in ms (crawl)")
	.option("-r --concurrency <integer>",	"Request concurrency (crawl)")
	.parse(process.argv);
