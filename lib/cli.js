...
function startCrawl() {
	...
	crawl.addFetchCondition(function(url) {
		if (url.uriPath.indexOf("/profiles/") === 0) return false;
		if (url.uriPath.indexOf("/sites/") === 0) return false;
		if (url.uriPath.indexOf("/misc/") === 0) return false;
		if (url.uriPath.indexOf("/files/") === 0) return false;
		if (url.uriPath.indexOf("/taxonomy/") === 0) return false;
...
function jsonToCSV(input) {
	...
	csvOut += "URL,Annotations,Selector,Message,Node,NodePath\n";
...
function printReport() {
	...
	if (!basCLI.json && !basCLI.csv) {
		...
		console.log(jsonToCSV(errorBatch));
