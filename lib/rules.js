...
var Assertion = function(rule) {
	this.annotations	= [];
	this.subject		= rule.property;
	this.assertion		= rule.value;
	this.position		= rule.position;
};

Assertion.prototype.test = function(documentState,tests,node) {
	var subjectParts	= this.subject.split(/[\(\,\)]/),
		subject			= subjectParts.slice(0,1),
		args			= subjectParts.slice(1);
	
	if (!this.tests[subject])
		throw new Error("Test " + subject + " not found in map.");
	...
	var testResult =
		this.tests[subject]
			.apply(
				this.tests[subject],
				[documentState,node].concat(args));
...
Assertion.prototype.toError = function() {
	return new Error("Oh no the assertion failed!");
};
