...
var Assertion = function(rule) {
		...
		if (component.match(/^["'].*["']$/)) {
			...
			return function(input) {
				...
				if (!result) {
					throw new Error(
						"'" + rawSubject + "' expects " + component + ": " +
						"Negated component test '" + bareword + "' failed " +
						"against input '" + helpers.trim(input) + "'."
					);
				}
			};
	...
	var assertion = function(testResult,returnErrors,url,nodePath,readableNode,selector) {
		var errorList = [];
		...
		assertions.forEach(function(assertion) {
			try {
				...
				if (!!readableNode)	e.node = readableNode;
...
Assertion.prototype.test =
	...
	var url = documentState.url,
		...
		readableNode = node ? helpers.getReadableNode(node) : "";
	...
	return this.assertion(testResult,!!returnErrors,url,nodePath,readableNode,selector);
};
